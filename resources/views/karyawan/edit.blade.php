...
@section('content')
    ...
    <div class="flex items-center justify-between mb-8">
        <div>
            <h2 class="text-2xl font-bold text-slate-800 tracking-tight">Edit Karyawan</h2>
            <p class="text-sm text-slate-400 mt-1">Perbarui data karyawan <span class="font-medium text-indigo-500">{{ $karyawan->nama }}</span></p>
        </div>
                ...
                <div class="mb-8">
                    ...
                    <select name="departemen_id" id="departemen_id" class="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl text-sm text-slate-700 focus:outline-none focus:ring-2 focus:ring-indigo-300 focus:border-indigo-400 transition-all duration-200 appearance-none cursor-pointer">
                        ...
                        @foreach ($departemens as $dept)
                            <option value="{{ $dept->id }}" {{ $karyawan->departemen_id == $dept->id ? 'selected' : '' }}>
